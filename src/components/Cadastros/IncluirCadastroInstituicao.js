    ...
    return (
        <div className="container animeLeft ">
            ...
            <div className='content' >
             {showAlertSucessInclude && <AlertSucess texto='Registro Incluído com Sucesso !' show={showAlertSucessInclude} className='col-md-12' onClick={() => setShowAlertSucessInclude(false)}></AlertSucess>} 
               {showAlertErrorInclude && <AlertError texto='Erro na Inclusão do Registro !'  show={showAlertErrorInclude} className='col-md-12'  onClick={() => setShowAlertErrorInclude(false)}></AlertError>} 
                <Form onSubmit={handleSubmit(onSubmit)} className="mt-4 ">
                    ...
                    <Form.Row>
                        <Input size='lg' lg='6' label='Graduação' name='graduationCode' value={graduationCode} type='text' register={register({ required: true })} textoErro={errors.graduationCode && "Campo Graduação é obrigatório"} placeholder='Nome do aluno' onChange={(event) => setGraduationCode(event.target.value)}></Input>
                    ...
                    <Form.Row>
                        <Input size='lg' lg='12' label='Responsável' name='responsible' value={responsible} type='text' register={register({ required: true })} textoErro={errors.responsible && "Campo Responsável é obrigatório"} placeholder='Nome do Responsável' onChange={(event) => setResponsible(event.target.value)}></Input>
                    ...
                    <Form.Row>
                        ...
                        <Input size='lg' lg='8' label='Email da Graduação' name='graduationEmail' value={graduationEmail} type='email' register={register({ required: true })} textoErro={errors.graduationEmail && "Campo Email da Graduação é obrigatório"} onChange={(event) => setGraduationEmail(event.target.value)}></Input>
                    </Form.Row>

                    <Form.Row>
                        <Input size='lg' lg='12' label='Email da Pós-Graduação' name='emailPostGraduation' value={emailPostGraduation} type='email' register={register({ required: true })} textoErro={errors.emailPostGraduation && "Campo Email da Pós-Graduação é obrigatório"} onChange={(event) => setEmailPostGraduation(event.target.value)}></Input>
                    </Form.Row>

                    <Form.Row>
                        <Input size='lg' lg='12' label='Endereço' name='address' value={address} type='text' register={register({ required: true })} textoErro={errors.address && "Campo Endereço é obrigatório"} onChange={(event) => setAddress(event.target.value)}></Input>
                    </Form.Row>
                    ...
                    <Form.Row>
                        <Select lg='6' size='lg' label='Estado' value={state} name='state' register={register({ required: true })} options={['Belo Horizonte', 'São Paulo', 'Rio de Janeiro']} textoErro={errors.city && "Campo Estado é obrigatório"} onChange={({ target }) => setState(target.value)}></Select>
                ...
                {table && dados.length > 0 && <div > <h5 className="mt-5"> Resultados Encontrados:</h5><Table striped bordered hover className=" col-lg-12  mt-3 animeLeft">
                    <thead >
                        <tr>
                            <th >ID</th>
                            <th>Instituiçao</th>
