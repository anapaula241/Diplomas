...
const DiplomaVisualizar = () => {
  ...
  return (
    <div className="container animeLeft ">
      ...
      {showAlertSuccessRegister && <AlertSucess texto='Registro Cadastrado com sucesso !' show={showAlertSuccessRegister} className='col-md-12' onClick={() => setShowAlertSuccessRegister(false)}></AlertSucess>} 
               {showAlertErrorRegister && <AlertError texto='Houve um erro no seu Cadastro !'  show={showAlertErrorRegister} className='col-md-12'  onClick={() => setShowAlertErrorRegister(false)}></AlertError>} 
        ...
        <Form onSubmit={handleSubmit(onSubmit)} className="mt-5 ">
          ...
          <Form.Row>
            <Input size='lg' lg="6" label='Naturalidade' name='nationality' register={register({ required: true })} value={nationality} type='text' textoErro={errors.nationality && "Campo Naturalidade é obrigatório"} placeholder='Naturalidade' onChange={(event) => setNationality(event.target.value)}></Input>
          ...
          <Form.Row>
            <Input size='lg' lg="4" label='Ano Conclusão ' name='yearConclusion' register={register({ required: true })} value={yearConclusion} type='text' textoErro={errors.yearConclusion && "Campo Ano Conclusão é obrigatório"} onChange={(event) => setYearConclusion(event.target.value)}></Input>
          ...
          <Form.Row>
            <Textarea label='Observação'  name = 'note' lg='12' rows={4} size='lg' className='mt-3'  register={register({ required: true })} onChange={({ target }) => setNote(target.value)} textoErro={errors.note && "Campo Situação é obrigatório"}></Textarea>
          </Form.Row>
